...
wrap_example = function(x, options) {
    paste0("\n\\begin{example}\n", x, "\n\\end{example}\n")
}

knit_hooks$set(source = wrap_example,
               output = wrap_example,
...
<<coefplot, dependson = "mtcars_by_cyl">>=
library(ggplot2)
...
<<dependson = "mtcars_by_cyl_regressions">>=
...
\begin{center}
<<regressions_plot, dependson = "regressions">>=
...
\begin{center}
<<survival_bootstrap_curve, dependson = "survival_bootstrap">>=
alpha = .05
...
<<save_session_info, include = FALSE, dependson = c("survival_bootstrap_curve", "purities_sd", "regressions_plot")>>=
{
